...
#' @details The output of this function is the same of the \code{\link[ntbbox]{cov_center}} function.
#' @export
#' @examples
#' \dontrun{
#' cluster_data <- read.csv(system.file("extdata/csv",
...
.plot_ellipsoid3d <- function(data,mve=F,color_index,alpha,level,colores,plot_clusters){
  ...
  if(mve){
    centroid_shape <- ntbbox::cov_center(data,mve=T,
                                        level=level,
                                        vars = 1:3)
  }
  else{
    centroid_shape <- ntbbox::cov_center(data,mve=F,
                                        level=level,
                                        vars = 1:3)
  }
