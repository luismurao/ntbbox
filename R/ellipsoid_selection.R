...
#' @details Model selection occurs in environmental space (E-space). For each variable combination the omission rate (omr) in E-space is computed using the function \code{\link[ntbbox]{inEllipsoid}}. The results will be ordered by omr and if the user-specified the environmental background "env_bg" an estimated prevalence will be computed and the results will be ordered also by "bg_prevalence".
#'
...
#' For more details and examples go to \code{\link[ntbbox]{ellipsoid_omr}} help.
#' @export
#' @import future
...
#' env_varsL <- ntbbox::correlation_finder(cor(pg_etrain),
#'                                        threshold = 0.8,
#'                                        verbose = F)
...
#' pg_proc <- ntbbox::pROC(continuous_mod = mProj$suitRaster,
...
ellipsoid_omr <- function(env_data,env_test=NULL,env_bg,cf_level,mve=TRUE,proc=FALSE,proc_iter=100,rseed=TRUE){
  emd <- try(ntbbox::cov_center(data = env_data,
                               mve = mve,
                               level = cf_level,
  ...
  if(!is.null(env_bg)){
    ...
    if(exists("in_etest")){
      ...
      d_results <-data.frame( d_results,
                              pval_bin=p_bin)
      if(proc){
        proc <- ntbbox::pROC(suits_bg,test_data = suits_val,
