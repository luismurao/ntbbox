...
#' @seealso \code{\link[ntbbox]{get_envirem_elev}}, \code{\link[ntbbox]{get_envirem_clim}},
#' \code{\link[ntbbox]{get_bio_oracle}}.
#' @details For more details visit \url{http://chelsa-climate.org/}
#' @references Karger, D.N., Conrad, O., Bohner, J., Kawohl, T., Kreft,
#' H., Soria-Auza, R.W., Zimmermann, N.E., Linder, H.P. & Kessler, M. (2017).
...
get_chelsa <- function(period,model=NULL,rcp=NULL,sv_dir=getwd(),load2r=TRUE,parallel=TRUE){
  ...
  if(period %in% c("2041-2060","2061-2080") && rcp %in% c("rcp26","rcp45","rcp85")){
    ...
    chelsa <- base::readRDS(file.path(system.file("extdata/rds",
                                                  package = "ntbbox"),
                                      "chelsa_vars.rds"))
    if(period == "2041-2060")
  ...
  if(is.null(chelsa_urls)){
    ...
    if(load2r)
      chelsa_urls <- ntbbox::rlayers_ntb(dir_name)
