...
#' @seealso \code{\link[ntbbox]{get_chelsa}}, \code{\link[ntbbox]{get_envirem_elev}},
...
get_envirem_clim <- function(period,gcm=NULL,region,resolution,fmt,sv_dir=getwd(),load2r){
  ...
  if(period=="current" || is.null(gcm)){
    clim_data <- base::readRDS(file.path(system.file("extdata/rds",
                                                     package = "ntbbox"),
                                         "envirem_clim_current.rds"))
  ...
  if(length(layer_ID)>0L){
    ...
    if(load2r)
      clim_stack <- ntbbox::rlayers_ntb(dirname)
